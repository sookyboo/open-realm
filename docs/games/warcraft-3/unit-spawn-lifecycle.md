...
Build the guarded AI diagnostics and run a bounded campaign launch:
...
For a runtime-created Town Hall, the bad sequence is a `birth` animation
request immediately followed by `stand` while `construction=0`. The corrected
`CreateUnit` path starts directly in `stand` and has no birth wait.
