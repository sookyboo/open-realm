...
Rendering deliberately reuses the splat path rather than uploading a regenerated RGBA texture each mouse frame. `client/tr_public.h:renderSplatRect_t` is a generic client→renderer scene primitive; `renderer/r_ents.c:R_DrawSplatRects()` batches submitted rectangles through `R_BeginSplatBatch()` / `R_AddRectSplat()` / `R_EndSplatBatch()`. Ordinary zero-initialized rectangles keep the terrain-conforming built-in-white behavior used by construction. The primitive also supports an optional texture plus `RSF_FIXED_Z` flat plane for diagnostics that must sit over world geometry instead of being projected back onto terrain; WC3's bridge path-texture visualizer uses that path as one quad rather than expanding a 32x32 TGA into 1,024 draw submissions. The placement colours use alpha 166/255 (approximately 65%). Warsmash uses about 20% for its inspected `MeleeUI.handleBuildCursor()` splat; OpenRealm deliberately uses the stronger opacity so blocked and buildable cells remain obvious against terrain and the placement model. This keeps the plan under the ghost model and avoids one draw/upload per pathing cell.
