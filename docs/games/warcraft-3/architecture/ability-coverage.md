...
| `Arai`, `ACrd`, `AIrd` | `s_undead_abilities.c`, `s_summon.c` | Partial; lower-rank-before-distance corpse priority, `Cantfindcorpse`, DataA×DataC plus DataB×DataD summons, authored BuffID presentation beside shared `BTLF` timed life, summon identity/effects, corpse consumption, autocast, and item alias are implemented. Meat Wagon real cargo and summon-cap parity remain. See [Corpse Mechanics](../corpse-mechanics.md). |
...
| `Arai`, `ACrd`, `AIrd` | Raise Dead | Partial | Lower-rank-before-distance corpse search, CommandStrings failure, two authored summon groups, timed life/authored buff, summon identity/effects, autocast, item alias, and corpse consumption exist. Meat Wagon cargo and summon-cap parity remain. |
