...
| `Acan` | `s_undead_abilities.c`, `m_unit.c` | Partial; nearest authored-target-valid corpse acquisition through DataB, including stored Meat Wagon cargo, exclusive decay-suspending reservation, continuous DataA HP/sec, full-health/duration/channel termination, corpse consumption on every started-channel end, and `Cantfindcorpse` feedback are implemented. Group-order wounded-unit assignment/retention and exact presentation remain. See [Corpse Mechanics](../corpse-mechanics.md). |
| `Agyd` | `s_undead_abilities.c`, `m_unit.c` | Partial; passive `Cool` cadence, DataA corpse cap, DataC corpse radius, UnitID corpse type, normal raisability/decay lifecycle, and saveable producer thinker are implemented. Exact retail random/grave-marker placement remains. See [Corpse Mechanics](../corpse-mechanics.md). |
| `Arai`, `ACrd`, `AIrd` | `s_undead_abilities.c`, `s_summon.c` | Partial; lower-rank-before-distance corpse priority, `Cantfindcorpse`, DataA×DataC plus DataB×DataD summons, authored BuffID presentation beside shared `BTLF` timed life, summon identity/effects, corpse consumption from world or Meat Wagon cargo, autocast, and item alias are implemented. Summon-cap parity remains. See [Corpse Mechanics](../corpse-mechanics.md). |
...
| `Acan` | Cannibalize | Partial | DataB nearest corpse search, authored corpse target filtering, exclusive reservation, continuous DataA healing, full-health/duration/interruption completion, corpse consumption, and `Cantfindcorpse` exist. Group-order policy and exact presentation remain. |
| `Agyd` | Create Corpse | Partial | Cool interval, DataA cap, DataC nearby-corpse radius, UnitID generation, shared corpse lifetime, and saveable producer state exist. Exact random/grave-marker placement remains. |
| `Arai`, `ACrd`, `AIrd` | Raise Dead | Partial | Lower-rank-before-distance corpse search, CommandStrings failure, two authored summon groups, timed life/authored buff, summon identity/effects, autocast, item alias, and corpse consumption from world or Meat Wagon cargo exist. Summon-cap parity remains. |
