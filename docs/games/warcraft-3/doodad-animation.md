...
Doodad model lookup follows the `Doodads.slk` `file` field directly.  A numeric variation suffix is appended only when `numVar > 1`; the legacy `dir` column is not used to reconstruct or duplicate the model path.  This matters for scripted animations because the game-side MDX loader must open the same authoritative model that the map renderer displays.
