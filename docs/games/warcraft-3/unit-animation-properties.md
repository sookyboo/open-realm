...
- `+set wc3_animation_debug 2` additionally samples frame progression once per second for focused presentation candidates: JASS/model effects, paused units, invulnerable pathing-disabled markers, and models whose paths identify Circle/Waypoint/Indicator/Beacon presentation art. Ordinary non-selectable terrain doodads/destructables are intentionally excluded. The sample records the driver (`monster`, `monster-paused`, `monster-no-move`, `monster-stunned`, or `effect`) and `frame=before->after`.
