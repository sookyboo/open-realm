...
`PanCameraToWithZ` and `PanCameraToTimedWithZ` author a camera-target Z offset in addition to X/Y. `G_RunClients()` interpolates that offset and the near/far clip planes alongside the other camera values, and the WC3 player snapshot transports all three to the client. The camera target Z is `max(terrain height, water height) + authored Z offset`: WC3 preset camera `Height` contributes no hidden one-tile subtraction, and targets over water follow the visible surface rather than submerged terrain. `Matrix4_getCameraMatrix()` treats the resulting snapshot origin as the orbit target rather than an absolute camera-eye position. Camera setups support `CAMERA_FIELD_NEARZ`, `CAMERA_FIELD_FARZ`, and `CAMERA_FIELD_ZOFFSET`; the `...WithZ` apply variants override the setup Z offset with their explicit argument. `CameraSetupApply(..., doPan=false, ...)` and `CameraSetupApplyForceDuration(..., doPan=false, ...)` preserve the current target position while still applying camera fields, matching Warsmash's separation between setup fields and destination panning.
