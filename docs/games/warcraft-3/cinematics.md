...
Camera and transmission natives should not grow unconditional investigative
tracing; use the focused JASS/network tests below or the compile-time-gated
camera diagnostics for a bounded campaign run. The pre-existing
`CLIENTCOMMAND(Cancel)` path still prints its ESC/end-cinematic event
publication.

For a camera mismatch, build the focused diagnostics and enable them for a
bounded run:

```sh
make -B openwarcraft3 WC3_DEBUG_CAMERA=1
build/bin/openwarcraft3 -data 'data/Warcraft III' +set wc3_camera_debug 1 \
  +map 'Maps\Campaign\Human02Interlude.w3m' +com_frame_limit 900 \
  2> /tmp/human02-camera.log
```

`WC3_CAMERA` lines from the game identify each JASS camera transition and its
position, angles, distance, lens, and pan interval. Client-side lines show the
camera matrix inputs after snapshot interpolation. If those values match while
models still intersect the view, the remaining issue is renderer camera
obstruction handling rather than map camera data. The diagnostics are
compile-time disabled unless `WC3_DEBUG_CAMERA=1` is supplied.
