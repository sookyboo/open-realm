...
Retail binary inspection confirms that destructables own a dynamic pathing overlay: `0x006d30b0` selects the alive/death profile pathing reference and the shared pathing-surface manager updates the registered footprint during both creation and restore. The final retail cell compositor beneath that manager is still unresolved. OpenRealm's current raster implementation in `common/routing.c` therefore remains a compatibility implementation: the authored image is sampled on its vertically flipped axis, uses the destructable's authored facing directly in 90-degree steps, is centred on the destructable's path cell, and is ORed into the WPM flags. The earlier extra `+90°` transform turned LT05's authored rail bands across the travel axis and is intentionally not used. A texture never clears a terrain restriction for ordinary destructables. Red, green, and blue source channels are read from the decoded TGA's `b`, `g`, and `r` members respectively; a channel contributes only when its value is greater than 127. Alive `walkable=1` destructables are the current bridge exception: their complete alive footprint is opened first so blocked water can become a crossing, then the authored alive path texture is stamped back over that opening so rail/non-deck pixels remain blocked. Dead pathing uses the authored death texture. The same static bake writes a compact full-footprint `walkable_surface_mask`, so movement, line checks, radius checks, and A* retain the working bridge support/radius semantics in O(1) without rescanning destructables and texture pixels.

Horizontal path baking must remain renderer-independent. Do not call the MDX walkable-surface mesh trace once per TGA pixel during `CM_BakeStaticObstacles()`; the authored path texture defines horizontal deck membership, while rendered-model tracing is reserved for vertical support after a mover is already on a baked walkable-surface cell. This separation avoids hundreds of renderer traces during LT05 restore.

Unreachable location orders also stay bounded. `CM_ClosestReachablePointForRadius()` uses a target-directed best-first search capped at 4096 expanded path cells instead of a synchronous whole-map heatmap plus full-map scan. Production reaches this fallback only after the destination-rooted flow has already proven the requested component unreachable; if the mover's component is larger than the cap, the fallback returns the closest legal point found within the budget rather than stalling the simulation thread.
