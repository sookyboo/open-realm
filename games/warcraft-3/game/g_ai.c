...
static LPCEDICT bridge_debug_find(LPCVECTOR2 point) {
    ...
    FOR_LOOP(i, globals.num_edicts) {
        ...
        if (fabsf(point->x - ent->s.origin2.x) <= range && fabsf(point->y - ent->s.origin2.y) <= range) return ent;
    }
    return NULL;
}

static BOOL bridge_debug_near(LPCVECTOR2 point) { return bridge_debug_find(point) != NULL; }

static BOOL bridge_probe_enabled(void) {
    LPCSTR value = gi.CvarString ? gi.CvarString("wc3_bridge_probe", "0") : "0";
    return value && atoi(value) != 0;
}

static void bridge_probe_move(LPCEDICT self, LPCVECTOR2 cand) {
    static DWORD count;
    LPCEDICT bridge;
    if (!self || !cand || !bridge_probe_enabled() || count >= 256) return;
    bridge = bridge_debug_find(cand);
    if (!bridge) bridge = bridge_debug_find(&self->s.origin2);
    if (!bridge) return;
    fprintf(stderr,
            "WC3_BRIDGE_PROBE_MOVE seq=%u unit=%08x bridge=%u from=(%.1f,%.1f) to=(%.1f,%.1f) radius=%.1f\n",
            count, self->class_id, (unsigned)(bridge - globals.edicts),
            self->s.origin2.x, self->s.origin2.y, cand->x, cand->y, self->collision);
    CM_DebugBridgeMoveProbe(&self->s.origin2, cand, self->collision);
    count++;
}
...
static void bridge_debug_move_reject(LPCEDICT self, LPCVECTOR2 cand, LPCSTR reason) {
    ...
    LPCEDICT bridge;
    BYTE flags = 0;
    if (G_BridgeDebugLevel() < 3 || count >= 256 || !(bridge = bridge_debug_find(cand))) return;
    CM_GetPathingFlagsAt(cand, &flags);
    fprintf(stderr, "WC3_BRIDGE_MOVE result=reject reason=%s unit=%08x from=(%.1f,%.1f) cand=(%.1f,%.1f) radius=%.1f flags=0x%02x\n",
            reason, self->class_id, self->s.origin2.x, self->s.origin2.y, cand->x, cand->y, self->collision, flags);
    CM_DebugPathingPoint(bridge, cand, "move_reject", G_BridgeDebugLevel());
    count++;
}

static void bridge_debug_move_accept(LPCEDICT self, LPCVECTOR2 cand) {
    static DWORD count;
    LPCEDICT bridge;
    BYTE flags = 0;
    if (G_BridgeDebugLevel() < 4 || count >= 256 || !(bridge = bridge_debug_find(cand))) return;
    CM_GetPathingFlagsAt(cand, &flags);
    fprintf(stderr, "WC3_BRIDGE_MOVE result=accept unit=%08x from=(%.1f,%.1f) cand=(%.1f,%.1f) radius=%.1f flags=0x%02x\n",
            self->class_id, self->s.origin2.x, self->s.origin2.y, cand->x, cand->y, self->collision, flags);
    CM_DebugPathingPoint(bridge, cand, "move_accept", G_BridgeDebugLevel());
    count++;
}
...
static BOOL move_is_valid_policy(LPEDICT self, LPCVECTOR2 cand,
    ...
    bridge_debug_move_accept(self, cand);
