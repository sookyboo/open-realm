...
static void UI_CopyFrameBase(LPUIFRAME dest, LPCFRAMEDEF src) {
    ...
    dest->flagsvalue |= src->ui_flags & (UIFLAG_EXTEND_WIDESCREEN_X | UIFLAG_PROGRAMMATIC_HIGHLIGHT);
...
static uiSimpleButton_t MakeSimpleButton(LPCFRAMEDEF frame) {
    ...
    LPCFRAMEDEF highlight = UI_ButtonPart(frame, frame->Button.UseHighlight);
    result.highlightAlphaMode = highlight ? highlight->AlphaMode : frame->AlphaMode;
