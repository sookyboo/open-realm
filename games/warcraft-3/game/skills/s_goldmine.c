...
void S_MineOverlayRelease(LPEDICT overlay) {
    ...
    FILTER_EDICTS(worker, worker->inuse && worker->acolyte_mine.mine == overlay &&
        ...
        if (worker->currentmove && worker->currentmove->ability == &a_acolyte_harvest)
            unit_stand(worker);
    }
...
static umove_t acolyte_harvest_move_walk = { "walk", ai_acolyte_harvest_walk, NULL, &a_acolyte_harvest };
static umove_t acolyte_harvest_move_work = { "stand work", ai_acolyte_harvest_work, NULL, &a_acolyte_harvest };
...
ability_t a_entangled_mine = {
    .update = S_EntangledMineTick,
};
