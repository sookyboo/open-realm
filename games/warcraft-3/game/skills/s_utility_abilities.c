...
static BOOL moon_well_validate(LPEDICT caster, spellTarget_t st) {
    LPEDICT target = st.entity;
    DWORD level = S_SpellLevel(caster, ID_MOON_WELL);
    FLOAT mana_ratio = S_SpellData(ID_MOON_WELL, level, 1); /* DataA: well mana / target mana */
    FLOAT life_ratio = S_SpellData(ID_MOON_WELL, level, 2); /* DataB: well mana / target HP */
    FLOAT available;

    if (!caster || !target || !S_SpellIsFriend(caster, target)) return false;
    available = MAX(0.0f, caster->mana.value);
    if (available <= 0.0f) return false;
    if (life_ratio > 0.0f && target->health.value < target->health.max_value) return true;
    if (mana_ratio > 0.0f && target->mana.value < target->mana.max_value) return true;
    return false;
}
