    ...
    FOR_LOOP(i, SCR_NumFrames()) {
        ...
        if (CL_WindowIsEditBox(frame) && frame->buffer.data &&
            ...
            fprintf(stderr,
                    ...
                    "selected=%d scroll=%.3f text=\"%s\"\n",
                    (unsigned)window->id, (unsigned)frame->number, (unsigned)frame->parent,
                    list->id, r->x, r->y, r->w, r->h, (unsigned)list->text.font,
                    list->itemHeight, list->border, (int)list->selectedIndex, frame->value,
...
LPCSTR CL_WindowEditTextValue(DWORD text_frame) {
    clientWindow_t *window;

    if (!text_frame) return NULL;

    /* Prefer the actively edited window, then fall back to any live transient
     * window that owns this text child. The server-authored STRING frame keeps
     * the initial value; typed characters live in clientWindowEdit_t until the
     * action is submitted, so rendering must consult this client-local state. */
    window = cl_windows.edit_window;
    if (window) {
        FOR_LOOP(i, window->num_edit_values) {
    ...
    for (window = cl_windows.first; window; window = window->next) {
        if (window == cl_windows.edit_window) continue;
        FOR_LOOP(i, window->num_edit_values) {
            clientWindowEdit_t const *value = &window->edit_values[i];
            if (value->text_frame == text_frame) return value->text;
        }
...
BOOL CL_WindowEditCursor(DWORD text_frame, LPDWORD cursor) {
    clientWindow_t *window = cl_windows.edit_window;
    if (!window || !text_frame) return false;
    FOR_LOOP(i, window->num_edit_values) {
        clientWindowEdit_t const *value = &window->edit_values[i];
