...
void SCR_LayoutDrawListBox(LPCUIFRAME frame, LPCRECT screen) {
    ...
    /* Follow the selectable cinematic/mission MapListBox convention: first
     * visible row starts at the top of the viewport and subsequent rows move
     * downward.  The transient-window hit test uses this exact geometry. */
    FLOAT item_y = list_rect.y;
    FLOAT const list_bottom = list_rect.y + list_rect.h;
    while (line && item_y < list_bottom) {
        char *hidden = strchr(line, '\t');
        if (hidden) *hidden = '\0';
        RECT row = { list_rect.x, item_y, list_rect.w,
                     MIN(item_height, list_bottom - item_y) };
