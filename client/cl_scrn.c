...
/*
 * SDL mouse positions are window pixels, while re.DrawSprite() renders in
 * the engine's virtual UI coordinate space. Keep one conversion helper so
 * cursor drawing and FDF hit-testing use exactly the same mapping.
 */
